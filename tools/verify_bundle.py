...
def main():
    ...
    with ZipFile(ROOT / 'hackalem-offline-py311.zip') as archive:
    ...
    start = perf_counter()
    # The shared launcher invokes pip with --no-index; no copied venv is available.
    result = subprocess.run([sys.executable, '-X', 'utf8', str(target / 'launch.py'), '--out', str(target / 'fresh-output')],
    ...
    compared = ['nodes_roles.csv', 'clusters.csv', 'top_nodes.csv', 'ranking_sensitivity.csv', 'graph.html', 'analytics.json', 'temporal_matches.parquet']
    ...
    report = {'clean_install_and_run_seconds': round(perf_counter()-start, 3),
              ...
              'pip_mode': '--no-index --find-links vendor/wheels/<platform>', 'python': sys.version.split()[0]}
